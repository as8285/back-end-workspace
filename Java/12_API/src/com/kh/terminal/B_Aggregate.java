...
public class B_Aggregate {
	...
	public static void main(String[] args) {
		...
		b.method2();
		/*
		 * 기본 집계 - 최종 처리 기능으로 요소들을 개수,합계,평균값,최대값,최소값 등과 같이 하나의 값으로 산출하는 역활 - count : 요소의
		 * 개수 리턴 - sum : 요소들의 합계 리턴 - average : 요소들의 평균 리턴 - max : 최대 요소 리턴 - min : 최소
		 * 요소 리턴 - findFirst : 첫번째 요소 리턴
		 */
	...
	public void method1() {
		...
		int sum = Arrays.stream(array).sum(); // int타입
		System.out.println("합 : " + sum);

		/*
		 * Optional클래스 - 스트림의 최종 결과 값을 저장하는 객체 - 단순히 값만 저장하는 것이 아니라, 값의 존재 여부를 확인하고 값이
		...
		/*
		 * int minResult = min.orElse(0); System.out.println("최소값 : " + minResult);
		 * System.out.println("최소값 : " + min.getAsInt());
		 */

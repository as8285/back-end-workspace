...
public class A_String {
    ...
    //  -전달받은 문자열고 ㅏ원본 문자열을 하나로 합친 새로운 문자열을 생성해서 리턴  
	String str2 = str.concat("!!!!!");
	System.out.println(str2);
	System.out.println("str2 : " + str2);
	
	// 3. substring(int beginIndex) : String  
	// 		- 문자열의 beginIndex 위치에서 부터 끝까지 문자열을 새로 생성해서 리턴 
	// substring(int beginIndex,int endIndex) : String 
	// - 문자열의 beginIndex 위체에서부터 endIndex-1 까지 
	String str3 = str.substring(6);
	String str4 = str.substring(2, 7);
	System.out.println(str3); // "Hello,Java !"; 6 번째 부터 출력 
	System.out.println(str4); // 2 ~ 7 까지 출력  
	
	// 4.indexOf(String str) : int 
	// - 전달받은 str의 시작 인덱스를 리턴(포함되어 있지 않으면 -1 )
	int index = str.indexOf("!");
	System.out.println(index);// Hello,Java !!!!! 느낌표까지 11 이라서 
	...
	System.out.println(str5); // Hecco,Java !
	...
	// 7 . trim() : String 
	// 		- 문자열의 앞뒤 공백을 제거한 새로운 문자열 생성해서 리턴   
	String str8 = "           Hello,            Java!                 ";
	System.out.println("trim 전 : " + str8);
	String str9 = str8.trim();
	System.out.println("trim 후 : " + str9); // 앞에서 제거 
	...
	System.out.println("str 10 : " + str10);
		...
		*/
		public void method3() {
			...
			//   - 기존 문자열 뒤에 문자열 추가 
			sb.append("World !"); // word가 추가 됬음  
		    // 2. insert(int offset, String str) : StringBuilder
			//	- 문자열의 offset 위치부터 전달받은 문자열 추가 
			sb.insert(1, "eeeee"); // 1번째에 추가 됨 Heeeeello, World !
		
			
			//3. delete(int star,int end) : StringBuilder
			// 	- start 에서 end-1까지의 인덱스에 해당하는 문자열 삭제 
			sb.delete(1, 6);
		     //eeeee 제거 됨 
	...
	System.out.println(sb2); //margorP avaJ
	...
	// IPA avaJ
	  ...
	  * - 객체 생성시 생성자로 전달받은 문자열을 구분자를 이용하여 분리  
	  * - 분리된 최소 단위를 토큰이라고 부른다.
	  * 
	  *  */
	public void method4() {
		String str= "HTML,CSS,JavaScript,MySQL,Java,JDC,Servlet,JSP,jQuery,MyBatis,Spring,React";
		...
		int i = 0;
		// CountTokens : 꺼내지 않고 남아있는  토큰을 수를 확인 
		
		System.out.println("과목 개수 : " + st.countTokens());
		System.out.println(st.hasMoreTokens());
		//System.out.println(st.nextToken());// HTML 꺼냄 
		//System.out.println(st.nextToken());// CSS 꺼냄  
		...
		System.out.println("꺼내고 남은 과목 개수 : " + st.countTokens()); // 10개 됨  
	}

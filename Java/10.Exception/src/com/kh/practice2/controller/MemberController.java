...
// 멤버 추가 
public void inserMember(Member m) throws DuplicateNameException, RecordNotFoundException {
	int index = checkId(m.getId());
	
	if (index == -1) {
		mArr[count++] = new Member(m.getId(),m.getName(),
				        m.getPassword(),m.getEmail(),m.getGender(),m.getAge());	
	}else {
		// 회원이 기존에 있는 경우
...
// 멤버가 있는지 아이디 조회 --> 에러발생 시키는 부분이 추정 / 수정 때가 다른기 때문에 쓰기가 애매하다.

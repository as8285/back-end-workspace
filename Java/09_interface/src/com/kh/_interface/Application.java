 ...
 * - 객체가 구현해야 하는 기능이 있을때 인터페이스에 추상 메서드를 추가해서 
 *   객체가 기능을 구현하도록 만들 수 있다.
 *   - 인터페이스를 구현하려면 implements 키워드를 사용한다.
 *   - 인터페이스는 다중 상속(구현)을 허용한다.
 *   - 추상 클래스보다 더 강한 규칙성,강제성을 가진 것이 인터페이스이다. 
 *   
 *   추상클래스와 인터페이스 
 *   공통점  
 *   
 *   - 객체 생성은 안되나,참조 변수르논 사용이 가능하다.(즉,다형성 적용 )
 *   - 상속(구현)하는 클래스에서 추상 메서드를 재정의 하도록 강제한다.
 *   
 *   차이점 
 *   - 추상 클래스는  abstract 키워드로 클래스가 정의되어 있고 클래스 내에 변수,메서드 ,생성 가능 
 *   - 인터페이스는 interface 키워드로 정의되어 있고 인터페이스내에 변수,메서드 생성 불가능  
 *   
 *   extends와 implements 
 *   - 클래스 간의 상속 관계 일때 : 자식 클래스 명 extends 부모 클래스명 
...
//	Vehicle v = new Vehicle(); <-- 인터 페이스 객체 생성 x 

...
public class FileReadingTest {
	...
	public void fileSave() {
		// FileWritter를 사용해서 파일을 생성하고 데이터를 문자 단위로 저장
		// try-with-resource 문을 통해서 사용한 리소스를 자동으로 close()
		try (FileWriter fw = new FileWriter(fileName, true)) { // true일경우 추가됨
			// try(FileWriter fw = new FileWriter(fileName,false)) 경우 삭제
	...
	public void fileRead() {
		// FileReader를 사용해서 파일에서 데이터를 문자 단위로 읽어 온다 .
		//
	...
	// 보조 스트림 BufferdReader :버퍼(char[]) 데이터가 쌓이기를 기다렸다가 꽉 차게 될 시 한거번에 보냄 
	public void method1() {
		try (FileReader fr = new FileReader(fileName); 
	...
	public void method2() {
		...
		/*
		 * Buffered Reader,BufferedWriter : 입출력 효율을 높이기 위해 버퍼
		 *                                  (char[])를 사용하는 보조스트림

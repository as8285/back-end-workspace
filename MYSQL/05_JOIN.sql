/*
	JOIN 
    - 두 개 이상의 테이블에서 데이터를 조회하고자할 때 사용하는 구문 
    - 조회 결과는 하나의 결과물(RESULT SET)으로 나옴 
    
    - 관계형 데이터 베이스는 최소한의 데이터로 각각의 테이블에 담고 있음 
    (중복을 최소하하기 위해 최대한 쪼개서 관리 )
    부서 데이터는 부서 테이블,사원에대한 데이터는 사원 테이블,직급 테이블.....
    만약 어떤 사원이 어떤 부서에 속해있는지 부서명과 같이 조회 하고 싶다면?
    ...
    2) ANSI(미국국립표준협회 산업표준을 제정하는 단체)표준구문 -> 다른 DB에서도 사용가능 
    SELECT      컬럼,컬럼....
    FROM        테이블1 
...
SELECT emp_id,emp_name,salary,dept_code,dept_title
FROM employee 
 JOIN department ON(dept_code=dept_id)
...
-- 4. 부서가 총무부가 아닌 사원명,급여 조회(employee,department)
...
/*
	2.외부 조인(OUTER JOIN)
    -두 테이블 간의 JOIN 시 일치하지 않은 행도 포함 시켜서 조회가 가능하다 
    - 단,반드시 기준이 되는 테이블(컬럼)을 지정해야한다.(LEFT,RIGHT,FULL)
*/
...
-- 3) FULL [OUTER] JOIN : 두 테이블이 가진 모든 행을 조회 할수 있음  // MY SQL 은 안됨 
SELECT emp_name,dept_title,salary,salary*12  
FROM employee  
...
/*
    ...
    - ANSI 구문으로 JOIN ON만 사용 가능!(USING 사용 불가)
*/
SELECT * FROM employee; 
SELECT * FROM sal_grade; -- 급여 등급 테이블 : sal_leve,min_sal,max_sal
...
-- 사번, 사원명, 부서명, 지역명 조회 (employee,department,location)
SELECT emp_id,emp_name,dept_title,local_name
FROM employee 
		JOIN department on (dept_code = dept_id)
        JOIN location ON  (location_id=local_code);
-- 사번, 사원명, 부서명, 지역명,국가명 조회 (employee,department,location,national)
SELECT emp_id,emp_name,dept_title,local_name,national_name
FROM employee 
		JOIN department on (dept_code = dept_id)
        JOIN location ON  (location_id=local_code)
        JOIN national USING (national_code);
-- 사번, 사원명, 부서명,직급명, 지역명,국가명 조회 (employee,jobdepartment,location,national)
...
FROM employee 
        ...
        JOIN job USING(job_code)
        JOIN national USING (national_code);
 -- 사번, 사원명, 부서명,직급명, 지역명,국가명,급여등급조회 (employee,jobdepartment,location,national,sal_grade)
...
-- employee       emp id,   emp_naem, dept_code,            job_code                     salary          
-- department     dept_title          dept_id,   location_id 
-- location       local_name          local_code                      national_code
-- national       national_nme                                        national_code
-- sal_grade
--                                                                             min_sal,max_sal
...
AND local_code = location_id AND 
job_name='대리' AND local_name LIKE '%ASIA%';
-- 2. 70년생 이면서 여자이고 , 성이 전 씨인 직원들의 직원명,주민번호,부서명,직급명 조회 
...
-- AND substr(emp_no,8,1)=2;
 AND emp_no LIKE '7_____-2%' AND 
 emp_name LIKE '전%';
-- AND emp_no LIKE'7%';

-- 3. 보너스를 받은 직원들의 직원명,보너스,연봉,부서명,근무지역 조회 
--    단,부서 코드가 없는 사원도 출력될 수 있게 ! OUTER JOIN 사용! 
SELECT dept_code,emp_name,bonus,salary*12 "연봉",
dept_title,local_name
FROM employee 
LEFT Join department ON (dept_code=dept_id)
LEFT JOIN location ON (location_id=local_code) -- 빠져나오기 떄문에 LEFT 를 한번 더쓴다. 
WHERE bonus IS NOT NULL;

SELECT dept_code,emp_name,bonus,format((salary + salary + bonus)* 12,0)"연봉",dept_title,local_name
FROM employee 
LEFT Join department ON (dept_code=dept_id)
LEFT JOIN location ON (location_id=local_code) -- 빠져나오기 떄문에 LEFT 를 한번 더쓴다. 
WHERE bonus IS NOT NULL;

-- where 구문 (부서 코드가 없는에 대한건 x )
SELECT dept_code,emp_name,bonus,format((salary + salary + bonus)* 12,0)"연봉",dept_title,local_name

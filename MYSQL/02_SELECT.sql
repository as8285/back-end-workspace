/*
   ...
   - 관계형(Relational) 데이터베이스
	...
	-SQL(Structured Query Languag):관계형 데이터베이스에서 데이터를 조회하거나 조작하기 위해사용하는 표준 검색언어
        -SQL 종류
        1.DML(DataManipulation Language) : 데이터조작어alter
          - 실제 데이터를 조작하기 위해 사용하는 언어 
            (INSERT : 추가, UPDATA:수정 DELETE:삭제)
...
SELECT * 
FROM  employee;
---- employee 테이블의 전체 사원들의(emp_id),이름(emp_name),
---- 급여(salary)만을 조회 
...
/*
	...
	컬럼 별칭
    컬럼 as 별칭 / 컬럼 as "별침" / 컬럼 별칭 / 컬럼"별칭"
    - 산술연산을 하면 컬럼명이 지저분해진다.
    이떄 컬럼명에 별칭을 부여해서 깔끔하게 보여줄 수 있다.
    -별칭을 부여할 떄 띄어쓰기 혹은 특수문자가 포함될 경우 
    반드시 큰따옴표()감싸줘야한다. 
*/
...
*/
-- employee 테이블에서 (emp_id),직원명(emp_name),급여(salary),
-- 단위(원)조회
SELECT
emp_id 사번,
emp_name 직원명,
...
*/
-- employee 테이블에 직급 (job_code)조회 
SELECT DISTINCT job_code
FROM employee;
-- employee 테이블에 부서(dept_code)조회 
SELECT DISTINCT dept_code
FROM employee;

-- 유의 사항!DISTINCT는 SELECT절 딱 한번만 기술 가능 
...
/*
  SELECT 컬럼,컬럼,......
    ...
    FROM 테이블명
    WHERE 조건식;
  - 조회하고자 하는 테이블로 부터 특정 조건에 만족하는 데이터만
	조회하고자 할떄 사용
  - 이때 WHERE 절에 조건식 제시
...
/*
    ...
    비교대상컬럼 LIKE '특정패턴'
     ...
     비교대상컬럼 LIKE '_문자_' => 비교대상컬럼값에 문자 앞과 뒤에 무조건 한글자씩 올경우 조회 
*/
...
-- 이름 중에 하 인 포함된 사원들의 사원명,주민번호(emp_no),전화번호(phone) 조회 
SELECT emp_name,emp_no,phone
FROM employee
WHERE emp_name LIKE '%하%'; 
-- 이름의 가운데 글자가 하인 사원들의 사원명,전화번호 조회(이름이 3글자)
SELECT emp_id,phone 
FROM employee
...
-- 전화번호의 3번째 자리가 1인 사원들의 사번, 사원명, 전화번호, 이메일 조회 
 ...
 -- 이메일중 _ 앞글자가 3글자인 사원들의 사번, 이름, 이메일 조회 
 -- ex) sim_bs@kh.or.kr , sun_di@kh.or.kr
 -- ESCAPE OPITION : 나만의 와일드 카드를 만들어서 사용!($ ,#,내마음대로)
 -- 위의 사원들이 이외의 사원들 조회 
 -- 논리부정연산자 : NOT 
 ...
 -- WHERE NOT email LIKE '___#_%' ESCAPE '#';  <이방식도 가능>
...
SELECT * 
FROM employee
WHERE job_code ='J7'OR 	job_cod = 'J2'
AND salary >= 2000000 ;
...
-- 3. 연봉(보너스포함X)이 3000만원 이상이고 보너스를 받지않은 사원들의 
--      사번(emp_id),사원명,급여,보너스(bonus) 조회 
SELECT emp_id,emp_name,salary,bonus,salary * 12 as "연봉"
FROM employee
WHERE salary * 12 >= 3000000 AND bonus IS NULL;
...
-- 보너스를 받지 않은 사원들의 사번,사원명,급여,일사일,보너스 조회 
SELECT emp_id,emp_name,salary,hire_date,bonus
FROM employee
WHERE salary >= 2000000 AND salary >= 5000000
AND hire_date>=('2001-01-01') AND bonus IS NULL;
-- 6. 보너스 포함 연봉이 NULL이 아니고 이름에 '하'가 포함되어 있는 
-- 사원들의 사번,사원명,급여,보너스 포함 연봉조회 (별칭)
-- 보너스 포함 연봉 : (SALARY + SALARY * BONUS)*12
SELECT emp_id,emp_name,salary,bonus,(salary + salary*bonus)*12 as "연봉조회"
FROM employee
...
FROM employee
-- ORDER BY bonus -- 보너스 기준 오름차순 정렬(null 맨앞)
ORDER BY 2 DESC; -- 보너스 기준 내림차순 정렬(null 맨뒤)

/*
LIMIT 
- ORDER BY 절 보다 뒤에 조건을 걸고 싶을 때 사용 
- 출력되는 행 수를 제한 하는 MYSQL 전용 비표준 구문 
- 데이터 양을 제한하고자 할때 유용
*/ 
...
LIMIT 5;
-- 페이징 처리! LIMIT 절은 두 개의 인수가 있을 수 있음! 
-- 첫번째 값은 오프셋(offset,0부터 시작) 시작 행을 지정, 
-- 두번째 값은 반환할 최대 행 수를 지정 
SELECT emp_name,salary
FROM employee
ORDER BY salary DESC
LIMIT 5,10; -- 5로 시작하고 6을 가져온다 라는 뜻 

...
-- emp_name은 not null 로 지정 했기 때문에 에러가 발생한다. 
...
-- emp_dept 테이블에 employee에서 부서코드가 D1인 직원의 사번,이름,부서코드,입사일 추가 하고 
INSERT INTO emp_dept
 SELECT emp_id,emp_name,dept_code,hire_date
 FROM kh.employee 
WHERE dept_code= 'D1'; 
-- emp_manager 테이블에서 employee에서 부서코드가 D1인 직원의 사번,이름,관리자 사번을 추가 
...
START transaction;
-- emp_salary 에서 선동일 사장의 급여를 7000000으로,보너스를 0.2로 하락 
UPDATE emp_salary 
SET salary= 7000000, 
bonus=0.2
 WHERE emp_name='선동일';
 -- 모든 사원의 급여를 기존급여에서 10프로 인상한 (기존급여 *1.1)으로 변경
 UPDATE emp_salary 

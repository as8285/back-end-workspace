...
INSERT INTO tb_book VALUES(4,'고딩 좀 아는사람','제러미 키신',17800,3);
...
member_pwd VARCHAR(13) NOT NULL,
...
AS SELECT
		 ...
		 member_id 아이디,
         enroll_date 대여일,
         adddate(enroll_date,interval 7 day) 반납예정일
FROM sample.tb_member;
SELECT*FROM tb_member_copy;
-- 6. 회원번호가 1번인 회원이 대여한 도서들의 도서명, 출판사명, 대여일, 반납예정일을 조회하시오.
SELECT * FROM  tb_publisher;  -- 123 pub_no
SELECT*FROM tb_member; -- 123 member_no
SELECT*FROM tb_rent;
SELECT*FROM tb_book; -- 325
